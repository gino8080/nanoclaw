...
export class WhatsAppChannel implements Channel {
  ...
  constructor(opts: WhatsAppChannelOpts) {
    this.opts = opts;
    const envVars = readEnvFile(['WHATSAPP_READ_ONLY']);
    this.readOnly =
      (
        process.env.WHATSAPP_READ_ONLY ||
        envVars.WHATSAPP_READ_ONLY ||
        ''
      ).toLowerCase() === 'true';
  }
  ...
  private async connectInternal(onFirstOpen?: () => void): Promise<void> {
    ...
    this.sock.ev.on('messages.upsert', async ({ messages }) => {
      for (const msg of messages) {
        try {
          ...
          // In read-only mode, save ALL messages with text content.
          // In normal mode, only deliver for registered groups.
          const groups = this.opts.registeredGroups();
          if (this.readOnly || groups[chatJid]) {
            const content =
    ...
    // Save messages from history sync (initial batch when connecting).
    if (this.readOnly) {
  ...
  async sendMessage(jid: string, text: string): Promise<void> {
    if (this.readOnly) {
      throw new Error('WhatsApp channel is in read-only mode');
    }
